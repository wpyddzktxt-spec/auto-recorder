#!/bin/bash
# v6.32: NO set -e — handle errors manually, timeout 124 is NORMAL
...
# v6.32: all chunks 600s (10 min) for longer recordings
CHUNK_DURATION=600
...
while true; do
    ...
    if streamlink --version >/dev/null 2>&1; then
        timeout ${CHUNK_DURATION} streamlink --hls-live-edge 3 --retry-streams 2 --retry-max 2 --stdout "$HLS_URL" best 2>/tmp/sl_error_${MODEL}.log | \
            timeout ${CHUNK_DURATION} ffmpeg -y -i pipe:0 -c copy -movflags +faststart -f mp4 "$OUTFILE" 2>/dev/null || true
    ...
    if [ "$RECORD_OK" != "true" ]; then
        rm -f "$OUTFILE"
        timeout ${CHUNK_DURATION} ffmpeg -y -reconnect 1 -reconnect_at_eof 1 -reconnect_streamed 1 -reconnect_delay_max 10 -i "$HLS_URL" -c copy -movflags +faststart -f mp4 "$OUTFILE" 2>/dev/null || true
        SIZE=$(stat -c%s "$OUTFILE" 2>/dev/null || echo 0)
        if [ "$SIZE" -gt 4096 ]; then
            RECORD_OK=true
        fi
    ...
    if [ "$RECORD_OK" = "true" ] && [ -f "$OUTFILE" ]; then
        ...
        if [ "$CHUNK_SIZE" -gt 4096 ]; then
            ...
            if [ -n "$BOT_TOKEN" ] && [ -n "$CHAT_ID" ]; then
                ...
                TOTAL_BYTES=$((TOTAL_BYTES + FILE_SIZE))
                echo "[$(date -u +%H:%M:%S)] ${MODEL}: chunk ${CHUNK_INDEX} saved (${FILE_SIZE} bytes, no TG creds)"
                rm -f "$OUTFILE"
                CHUNK_INDEX=$((CHUNK_INDEX + 1))
                RETRY_COUNT=0
            fi
